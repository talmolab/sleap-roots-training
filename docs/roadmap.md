...
## Intern tracks (who leads — complementary, both work across)

- **Engineering lead (Anirudh):** pipeline architecture + tooling (Tiers 0–3, 7 tooling), then
  the mask-review GUI (Tier 8). Also labels real batches and co-reviews masks.
- **Modeling & evaluation lead (Natalie):** runs training/sweeps + evaluation, the
  generalist-vs-specialist comparison, labeling strategy/QC, trait validation (Tiers 3–6, 9).
  Also contributes pipeline PRs.
- **Cross-training is a requirement, not a hope:** Natalie ships **≥1 engineering PR** to the
  pipeline (e.g. config schema / data-loading utility, Tiers 2–3); Anirudh ships **≥1
  modeling/eval PR** (e.g. QC tooling, Tier 5) and labels a real batch. **Every PR is
  cross-reviewed** — Anirudh on the engineering angle, Natalie on the modeling/domain angle.
...
- **Shared onboarding (both interns):** each **labels a small batch** *and* **reproduces one
  training run end-to-end**.
- **Oracle:** CI green; package installs; `sleap-roots-training --help` runs; OpenSpec
  initialized and an empty change validates; old repo archived; both interns completed onboarding.
- **Lead:** Anirudh (onboarding shared). **Tracking:** EPIC issue in the new repo.
...
- **Secure SLEAP-team buy-in for `sleap-app` #155** Phase-1 scope (render + accept/reject):
  Anirudh drafts the scope as a comment/sub-issue and gets **written sign-off including an expected
  PR-review turnaround**. If no go-ahead, Tier 8 drops to a contingency / Phase 3 — **it is not on
  the mask-training critical path** (see Tier 7). During Phase 1, Anirudh **ramps on `sleap-app`**
  (reads issues/code, optionally lands a small non-blocking PR) so Phase 2 isn't a cold start.
- **Confirm repo name** `talmolab/sleap-roots-training`.
- **Oracle:** documented keypoint train/predict run; pins locked; #155 scope acknowledged by the
...
- **Oracle:** establish a **PyTorch-native baseline** (2–3 runs to get a stable range) on the
  ...
  reported alongside for context. *(W&B versioning is retrofitted in Tier 2.)*
- **Lead:** Anirudh (build); Natalie runs it. **Tracking:** Tier-1 EPIC.
...
- **Lead:** shared (a good home for Natalie's engineering PR). **Tracking:** Tier-2 EPIC.
...
- **Lead:** Natalie (domain) + Anirudh (checks). **Tracking:** Tier-2.5 EPIC. *(Full QC tooling
...
- **Lead:** Natalie (selection + eval) + Anirudh (resampling + sweep tooling).
  **Tracking:** Tier-2.7 EPIC. *(Tier 3 generalist training depends on this.)*
...
- **Deliverable:** train a generalist model across ≥2 species **on the unified skeleton (Tier
  ...
  with Natalie + Elizabeth to scope Tier 4.
...
- **Lead:** Natalie runs; Anirudh tooling. **Tracking:** Tier-3 EPIC.
...
- **Lead:** Natalie (co-owned harness with Anirudh). **Tracking:** Tier-4 EPIC.
...
- **Lead:** shared — **Anirudh's modeling/eval-side PR home** + Natalie domain.
  **Tracking:** Tier-5 EPIC.
...
- **Lead:** Anirudh glue; Natalie reviews. **Tracking:** Tier-6 EPIC. *(Re-verify sleap-nn mask
  state + pins at kickoff.)*
...
- **Lead:** Natalie runs; Anirudh tooling. **Tracking:** Tier-7 EPIC.
...
### Tier 8 — Mask review/correct GUI in `sleap-app` (#155 Phase-1) *(cross-repo, off critical path)*
...
- **Lead:** Anirudh + SLEAP team. **Tracking:** ties to existing `sleap-app` #155; buy-in secured
...
- **Weekly trio check-in** (Elizabeth + both interns, ~30 min): blockers, next-tier kickoff plan,
...
- **Right-size the per-tier review:** keep the adversarial OpenSpec *proposal* review, but run it
  **light for the straightforward tiers (1–3)** and **full-depth for the complex/cross-repo tiers
  (4 and 8)**. Cross-review between interns is a ~30-min async "other-angle" check, not a gate.
...
- **Cross-PR / cross-review requirement** (see Intern tracks) is tracked here too.
...
- Phase boundary timing (summer→fall, Natalie's hours).
