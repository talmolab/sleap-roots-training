...
2. **Understand scope**: First read `openspec/project.md` to ground yourself in this project's purpose, tech stack, architecture patterns, and domain context. Then use subagents to explore the codebase and understand the current state relevant to this feature. Investigate:
   - Existing modules under `src/sleap_roots_training/` and how they are structured
   - Config schema / OmegaConf definitions and how experiments are wired together
   - The CLI entry point in `src/sleap_roots_training/cli.py`
   - Test fixtures and test data under `tests/`
   - Related specs in `openspec/specs/`
   - Active changes in `openspec/changes/`

3. **Ask clarifying questions**: Based on what you learned from the codebase exploration AND `openspec/project.md`, ask the user any clarifying questions about:
   - Requirements and expected behavior
   - Edge cases (empty arrays, NaN, single points, missing config fields)
   - Biological validity and scientific accuracy (where the feature touches trait/skeleton/model semantics)
   - Data handling, coordinate systems, and units
   - Impact on reproducibility, published results, or model baselines
   - Memory / compute considerations (GPU, large datasets, slow training/inference)
   - Which species / crops / root types are affected
   - Which test data or fixtures are relevant
...
6. **Reconcile every blocking finding before user approval**: For each BLOCKING and IMPORTANT finding from the review, produce an explicit reconciliation entry containing:
   - The finding quoted verbatim — especially any specific technical mechanism named by the reviewer (e.g., "round-trip through a synthetic `.slp` file", "use `sio.load_slp`", "validate the OmegaConf schema with the real loader")
...
9. **Reconcile implementation with proposal before committing**: After implementation but BEFORE running `/pre-merge-check` or creating a commit, re-read the approved `proposal.md`, `spec.md`, and `tasks.md`. For every described behavior, data structure, and mechanism, verify the actual implementation matches. In particular:
   ...
   - If the spec says "round-trip through `.slp`", verify the test actually calls `sio.save_slp` and `sio.load_slp` (or the real loader)
