...
All examples derive the `owner/repo` dynamically with
`gh repo view --json nameWithOwner -q .nameWithOwner`, so this command works in any repo.

## Quick Usage (GraphQL - Recommended)

```bash
# Resolve owner and name dynamically
...
```bash
# View Copilot comments for a specific PR (REST API), repo resolved dynamically
gh api repos/$(gh repo view --json nameWithOwner -q .nameWithOwner)/pulls/PR_NUMBER/comments --jq '.[] | "File: \(.path):\(.line // .original_line)\n\(.body)\n" + ("="*80)'
...
# REST API
gh api repos/$(gh repo view --json nameWithOwner -q .nameWithOwner)/pulls/PR_NUMBER/reviews --jq '.[] | select(.user.login | contains("copilot")) | {state: .state, submitted_at: .submitted_at, body: .body}'
```
...
```
File: src/sleap_roots_training/config.py:310
...
================================================================================
File: src/sleap_roots_training/training.py:56
