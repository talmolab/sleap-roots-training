...
- [x] 1.1 Write the failing **oracle** test (`tests/test_config.py`): a minimal valid config — a
...
- [x] 1.2 Write the failing tests for `experiment`-metadata rejection: `experiment.species`,
      ...
      silently dropped.
- [x] 1.3 Confirm the tests fail (module not implemented yet).
- [x] 1.4 Implement `sleap_roots_training/config.py`: `ExperimentConfig` (OmegaConf structured,
...
- [x] 1.5 Confirm green; add the lazy-import lock test: after `import sleap_roots_training.config`
...
- [x] 2.1 Write the failing tests: `validate <good.yaml>` exits 0 with a success message (Click
...
- [x] 2.2 Confirm the tests fail.
- [x] 2.3 Implement the `validate` subcommand in `cli.py` (`@main.command(name="validate")`,
...
- [x] 3.1 Write the failing tests: a config that omits `trainer_config.seed`, sets it to `null`, or
...
- [x] 3.2 Implement the seed check in `validate_config`; confirm green.
- [x] 3.3 Write the failing **base-safe** skip-note test: with deep validation forced unavailable
...
- [x] 4.1 Write the failing tests: `trainer_config.use_wandb = true` without `wandb.entity` or
...
- [x] 5.1 Add `examples/arabidopsis_primary_cylinder.yaml`: a seeded, fully-resolved example whose
...
- [x] 5.2 Add a base-safe test asserting every `examples/*.yaml` passes the metadata + seed + W&B
      checks (guards the shipped example against rot; lives in `tests/`, inside the CI paths filter).
- [x] 5.3 Add `docs/training.md` (config authoring → `validate` → the single `sleap-nn train
...
- [x] 5.4 Add the doc-contract test (`tests/test_training_docs.py`, mirroring
...
- [x] 5.5 Add `docs/**` and `examples/**` to the `pull_request` **and** `push` `paths` lists in
...
- [x] 5.6 Update `openspec/project.md` Architecture Patterns (one line) from the day-0 bespoke-schema
