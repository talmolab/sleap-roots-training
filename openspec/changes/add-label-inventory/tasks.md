...
- [x] 1.1 Build the labels fixture tree into `tmp_path` (never committed — `.gitignore:53` is
...
- [x] 2.1 Create `src/sleap_roots_training/inventory/__init__.py` and `discover.py`.
- [x] 2.2 Test the exclusion rules: a `*.predictions.slp` file outside every derived
...
- [x] 2.3 Implement the walk and the exclusion rules. Pass `onerror=` so unreadable
...
- [x] 2.4 Test the family key: all four version-suffix shapes (`.vNNN.slp`, `.vNNN.pkg.slp`,
...
- [x] 2.5 Implement family grouping on (directory, basename minus only the `.vNNN` token,
      full suffix chain).

## 3. Reading the labels files

- [x] 3.1 Test: skeleton names, node names, node count, frame count, both instance counts and
      the referenced video filenames are recorded for a fixture file.
- [x] 3.2 Test: zero skeletons and more than one are reported distinctly. Branch on
...
- [x] 3.3 Test: a genuinely corrupt candidate (truncated bytes / non-HDF5 garbage) is recorded
...
- [x] 3.4 Implement `read.py` using `Labels.n_user_instances` and `Labels.n_pred_instances`.
...
- [x] 4.1 Test: a base64 MD5 equal to the matched entry's digest reports verified; a differing
...
- [x] 4.2 Implement `verify.py` against the per-file `ArtifactManifestEntry.digest`, reading
...
- [x] 5.1 Test: two families differing only in capture mode and node count are reported as
...
- [x] 5.2 Test: a derived species with no row is listed as uncovered. Inject the table via
...
- [x] 5.3 Test: an auto-generated `Skeleton-N` name is reported as unresolvable by the
      existing `partition("_")` check — it yields `('Skeleton-1', '', '')`, an empty root type.
- [x] 5.4 Implement `gap.py`, deriving mode from the family's directory path and node counts
      from the files — no external service.

## 6. Redaction

- [x] 6.1 Test: a candidate beneath the root emits with the fixed root token, and no segment
      of the root's absolute path appears anywhere in the artifacts.
- [x] 6.2 Test: a referenced video path emits as its filename alone, for both `C:\Users\...`
...
- [x] 6.3 Test: a candidate that cannot be expressed relative to the root — including a
      relative path and a `..`-rooted one — is reported by filename with the reason recorded.
- [x] 6.4 Test: `genotype`, `accession_id` and `experiment_name` survive unchanged.
- [x] 6.5 Implement `redact.py` as root-relativization plus basename extraction. Do not port
...
- [x] 7.1 Test: two runs over an unchanged tree emit byte-identical artifacts under
...
- [x] 7.2 Test: rows sort by emitted path, and an unresolvable group is listed as candidates
      with no collection membership asserted.
- [x] 7.3 Test: the only files written are the table and the report; the only files read are
      labels files, `skeletons.yaml` and registry manifests.
- [x] 7.4 Test: a name-derived species is labelled name-derived and stated not to be evidence.
- [x] 7.5 Test: a scan recording mismatches and unregistered families exits zero; a missing or
      unreadable root exits non-zero without emitting a partial table.
- [x] 7.6 Implement `emit.py` — one table, one report, under `inventory/`.

## 8. CLI

- [x] 8.1 Test: `inventory labels` over the fixture tree produces both artifacts and the
      documented exit statuses.
- [x] 8.2 Add the `inventory` group and its `labels` command to `cli.py` after
...
- [x] 9.1 Add a `docs/CHANGELOG.md` `### Added` entry under `[Unreleased]` and a one-line
...
- [x] 9.4 Validate strict on **both** pinned CLIs — 1.5.0 and 1.11.0 — and paste both
      ...
      silently.
- [x] 9.5 Flip `docs/superpowers/specs/2026-09-15-label-inventory-v2-scope.md`'s header from
