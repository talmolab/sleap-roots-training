# labeling-skeletons Specification

## Purpose
Define the committed per-crop labeling skeleton table (`skeletons.yaml`): what node count a
labeler must use so a new labeling package matches the corpus it extends, keyed by species,
capture mode, root type and age window. The table is descriptive of the native skeletons,
not Tier 2.7's unified one. A lookup that cannot choose one row raises rather than picking,
and each labeling package is built against its own capture mode.
