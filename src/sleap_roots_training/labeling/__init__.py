...
**deterministic and monotone** — widening a selection yields a superset of the narrower
one — so the "re-derive and republish" path for adding frames to an already-published
scan produces a genuine superset. And the built ``.slp`` **embeds its images**, so a
